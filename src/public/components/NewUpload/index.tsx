...
class _Upload extends Component<Props, State> {
    ...
    if (isLt && isTp) {
      ...
      reader.onload = function () {
        if (that.props.callback) {
          that.props.callback(file.name, reader.result, that.props.uploadName)
        }
  ...
  handleChange = ({ file, fileList, event }) => {
    ...
    if (this.state.fileShow) {
      ...
      this.setState({ fileList: [] })
      if (this.props.callback) {
        this.props.callback([], '', this.props.uploadName)
      }
    }
  }
  ...
  componentDidMount() {
    ...
    if (fileUrl && fileUrl !== 'undefined') {
      ...
      if (callbackOnInit) {
        callback('', fileUrl, this.props.uploadName)
      }
    } else {
      this.setState({
        fileList: []
  ...
  render() {
    ...
    const uploadButton = (
      <div style={{ backgroundColor: this.props.theme === 'light-theme' ? '#fff' : 'transparent', width: '100%', height: '100%' }}>
        <PlusOutlined style={{ marginTop: '25%', fontSize: '24px', color: this.props.theme === 'light-theme' ? 'rgba(5, 10, 25, 0.45)' : '#fff' }} />
        <div style={{ marginTop: 8, color: this.props.theme === 'light-theme' ? 'rgba(5, 10, 25, 0.45)' : '#FFF', whiteSpace: 'nowrap' }}>{this.props.uploadName}</div>
      </div>
    )
    return (
      <div className={classnames(this.props.className, 'clearfix')} style={{ display: 'inline-block', float: 'left', marginRight: 4 }}>
        {fileList.length >= 1 ?
