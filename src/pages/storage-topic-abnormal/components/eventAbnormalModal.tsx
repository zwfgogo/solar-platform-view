...
const EventAbnormalModal: React.FC<Props> = ({
  dispatch,
  visible,
  eventTableList,
  tableLoading,
  ...
  const fetchData = (date: Moment[]) => {
    dispatch({
      type: "abnormal/getEventAbnormalTable",
      payload: {
        stationId,
  ...
  const fetchExportData = () => {
    return { results: formatExportData(eventTableList, columns) };
  };
...
const mapStateToProps = state => ({
  ...
  tableLoading: state.loading.effects["abnormal/getEventAbnormalTable"]
});
export default connect(mapStateToProps)(EventAbnormalModal);
