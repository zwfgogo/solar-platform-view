...
const C05StrategyInfo: React.FC<Props> = (props) => {
  ...
  const onSearch = () => {
    if (selectEnergyUnitId) {
      sendToFetchData(stationId, timeStr, {
        ...
        analogsType: AnalogTypeNameByTypeMap[TrackingChartType.C05]
      })
    } else {
  ...
  return (
    <FullContainer className="c05-strategy-info run-strategy-info-page">
      <CommonHeader
        title={utils.intl('strategy.储能调峰')}
        fontSize={16}
...
export default makeConnect(storage_run_strategy_info, mapStateToProps)(C05StrategyInfo)
