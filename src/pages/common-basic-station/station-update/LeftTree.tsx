...
import { Tree_Type, Mode, Tree_Name } from '../../constants'
import { ActionProp } from '../../../interfaces/MakeConnectProps'
import { findTreeItem, getAllParentNodeByKey, getChangedKeys, traverseTree } from '../../page.helper'
...
export interface LeftTreeRef {
  getTreeRef: () => React.MutableRefObject<Tree>
  scrollTo: (key: string) => void
  loadData: (node, callback?: any) => Promise<any>
}
...
const LeftTree = React.forwardRef<LeftTreeRef, Props>(function (props, ref) {
  ...
  const onSelect = (keys, event) => {
    ...
    if (name == Tree_Name.gridConnectedUnit || name == Tree_Name.powerGrid) {
      return
    }
    if (type) {
      ...
      props.onSelect && props.onSelect(event.node)
      if (type !== Tree_Type.virtualNode) {
        if (type == Tree_Type.station) {
          props.action('fetchStationBasicInfoDetail', { stationId: props.stationId })
        } else if (type == Tree_Type.energyUnit) {
          props.action('fetchEnergyUnitDetail', { id: id })
        } else {
          props.action('fetchDeviceDetail', { deviceId: id })
        }
      } else {
        props.handleTitle(Mode.look, event.node.title)
        // props.action('updateState', { // 点击设备大类时也需要修改面包屑的展示
        //   detail: event.node
        // })
      }
    }
  ...
  const handlerTree = (treeList => {
    ...
    return treeList.map(item => {
      ...
      if (leaf.type === Tree_Type.virtualNode) {
  ...
  useEffect(() => {
    if (JSON.stringify(props.treeList) !== JSON.stringify(treeList)) {
  ...
  const scrollTopView = (key) => {
    const antdTreeRef = treeRef.current?.treeRef
    if (antdTreeRef) {
      antdTreeRef.current?.scrollTo({ key })
    }
  }

  const handleScrollTo = (key) => {
    const fn = (list) => {
      const parentKeys = getAllParentNodeByKey(list, key)
        .map(node => node.key);
      const keyMap = {}
      parentKeys.forEach(key => keyMap[key] = true)
      let newUserCloseKeys = userCloseKeys.filter(key => !keyMap[key])
      setUserCloseKeys(newUserCloseKeys)
      setTimeout(() => {
        scrollTopView(key)
      })
    }
  ...
  useImperativeHandle(ref, () => ({
    getTreeRef: () => treeRef.current?.treeRef,
