...
export default makeModel('stationModel', new station(), (updateState, updateQuery, getState) => {
    return {
        ...
        * onExport(action, { call, put, select }) {
            const res = yield Service.getList({});
            exportFile(getColumns(), res.results)
        }
    }
})

function getColumns() {
    const station_columns: ExportColumn[] = [
        {
        ...
        {
            title: utils.intl('电站类型'), dataIndex: 'title'
        },
        {
