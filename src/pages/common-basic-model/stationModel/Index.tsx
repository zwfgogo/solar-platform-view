...
class Station extends React.Component<any> {
    ...
    showBj = (record) => {
        ...
        dispatch({
            ...
            payload: { record: record, addModal: true, modalTitle: '编辑电站类型' }
        })
    };
    ...
    showDetail = async (record) => {
        ...
        dispatch({
            type: 'stationModel/updateState',
            payload: { record: record, detailModal: true, modalTitle: '查看电站类型' }
        })
    };
    ...
    delete = (id) => {
        const { dispatch } = this.props;
        dispatch({
            type: 'stationModel/deleteRecord',
            payload: {
                id: id,
            },
        });
    ...
    render() {
        ...
        const columns: any = [
            ...
            {
                title: utils.intl('电站类型ID'), dataIndex: 'name'
            },
            {
                title: utils.intl('模型电站类型'), dataIndex: 'title', width: 200,
            },
            {
            ...
            {
                title: utils.intl('可包含能量单元类型'), dataIndex: 'batteryCapacity', key: 'gdms', width: 200,
                render: (text, record, index) => {
                    return (
                        <span>
                            <Forward to="contentType" data={{ contentRecord: record, pageTitle: '模型可包含能量单元类型' }}>
            ...
            {
                title: utils.intl('常用电站参数'), dataIndex: 'batteryCapacity', key: 'gdms', width: 130,
                render: (text, record, index) => {
                    return (
                        <span>
            ...
            //         return (
            //             <span>
            //                 <Forward to="detail" data={{ _id: record.id, _deviceTitle: record.title, _type: 'station' }}>
...
export default makeConnect('stationModel', mapStateToProps)(Station)
