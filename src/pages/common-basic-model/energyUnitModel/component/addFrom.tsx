          ...
          <Row>
            <Col span={12}>
              <FormItem
                name="name"
                {...formItemLayout}
                label={utils.intl('能量单元类型ID')}><Input disabled autoComplete="off" /></FormItem>
            </Col>
            <Col span={12}>
              <FormItem
                name="title"
                rules={[{ max: 30, required: true, message: utils.intl('请输入30字符以内能量单元类型') }, startWithChineseLetterOrNumberRule(), checkTextDataForNORMA()]}
                {...formItemLayout}
                label={utils.intl('能量单元类型')}><Input autoComplete="off" /></FormItem>
            </Col>
            <Col span={24}>
...
export default makeConnect('energyUnitModel', mapStateToProps)(_Form)